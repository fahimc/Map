...
(function() {
	...
	_.onMouseWheel = function(event) {
		...
		var ps = this.scale;
		 ...
		 var px = this.x;
		 var py = this.y;
		//this.centerAroundMouse(x,y,ps,px,py);
		 this.setScale();
	...
	_.centerAroundMouse=function(x,y,ps,px,py)
	{
		//work out the increase
		var scaleDiff = this.scale - ps;
		var newXPos  = this.x + (this.x * scaleDiff);
		var newMouseX  = x + (x * scaleDiff);
		var newYPos  = this.y + (this.y * scaleDiff);
		var newMouseY  = y + (y * scaleDiff);
		var currentXPos = -(Math.abs(newXPos)+newMouseX);
		var currentYPos = -(Math.abs(newYPos)+newMouseY);
		
		this.x=currentXPos+(this.element.clientWidth * 0.5);
		this.y=currentYPos+(this.element.clientHeight * 0.5);
	};
